...
into the indexer weights, and the fp8 K quant + paged K-cache write. On
gfx942/gfx950 with AITER one kernel does all five (kill switch:
VLLM_ROCM_USE_AITER_INDEXER_QK_FUSION=0).
...
def main() -> None:
    ...
    for num_tokens in args.num_tokens:
        ...
        # fp32 on AITER >= v0.1.19.post1 (what vLLM's LayerNorm stores), q's
        # dtype before that.
        norm_dtype = aiter_indexer_qk_fusion_norm_dtype(dt)
        norm_w = torch.randn(HEAD_DIM, device=dev, dtype=norm_dtype)
        norm_b = torch.randn(HEAD_DIM, device=dev, dtype=norm_dtype)
