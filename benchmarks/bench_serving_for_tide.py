...
def get_accept_length_weight_version_from_server(
    ...
    if "sglang" in backend:
        ...
        # For vLLM, prefer using before/after deltas later (see fetch_spec_decode_metrics below).
        accept_length = None
        weight_version = None
    else:
        accept_length = None
...
async def benchmark(
    ...
    # Spec decode metrics fetcher (vLLM: /metrics, sglang: /get_server_info)
    def fetch_spec_decode_metrics(base_url: str, backend: str):
        try:
            if "vllm" in backend:
            ...
            if "sglang" in backend:
                ...
                if "decode" in info:
                    info = info["decode"][0]
                return {
                    "num_drafts": info["internal_states"][0].get("num_drafts", 0),
                    "num_draft_tokens": info["internal_states"][0].get("num_draft_tokens", 0),
                    "num_accepted_tokens": info["internal_states"][0].get("num_accepted_tokens", 0),
                    "accepted_per_pos": {},
                }
        except Exception:
            return None
    ...
    if not log_interval:
        ...
        spec_decode_prev = None
        if "vllm" in backend:
            spec_decode_prev = fetch_spec_decode_metrics(base_url, backend)
        ...
        while pending_tasks:
            ...
            if done:
                ...
                if outputs and len(outputs) > last_logged_completed:
                    ...
                    weight_version = None
                    if "vllm" in backend:
                        spec_decode_curr = fetch_spec_decode_metrics(base_url, backend)
                        if spec_decode_prev and spec_decode_curr:
                            delta_drafts = (
                                spec_decode_curr.get("num_drafts", 0)
                                - spec_decode_prev.get("num_drafts", 0)
                            )
                            delta_accepted = (
                                spec_decode_curr.get("num_accepted_tokens", 0)
                                - spec_decode_prev.get("num_accepted_tokens", 0)
                            )
                            if delta_drafts > 0:
                                accept_length = 1 + delta_accepted / delta_drafts
                        spec_decode_prev = spec_decode_curr
                    else:
                        accept_length, weight_version = (
                            get_accept_length_weight_version_from_server(
                                backend, base_url
                            )
                        )
                    print_metrics(
    ...
    # --- Spec decode metric 측정: 벤치마크 시작/종료 시점의 서버 상태 받아오기 ---
    spec_decode_before = fetch_spec_decode_metrics(base_url, backend)

    # ...기존 벤치마크 실행 코드...

    # 벤치마크 종료 후 다시 받아오기
    spec_decode_after = fetch_spec_decode_metrics(base_url, backend)

    acceptance_length = None
    if spec_decode_before and spec_decode_after:
        delta_drafts = spec_decode_after.get("num_drafts", 0) - spec_decode_before.get("num_drafts", 0)
        delta_draft_tokens = spec_decode_after.get("num_draft_tokens", 0) - spec_decode_before.get("num_draft_tokens", 0)
        delta_accepted = spec_decode_after.get("num_accepted_tokens", 0) - spec_decode_before.get("num_accepted_tokens", 0)
        if delta_drafts > 0:
            acceptance_length = 1 + delta_accepted / delta_drafts

    # 기존 weight_version 로직 유지
    _, weight_version = get_accept_length_weight_version_from_server(backend, base_url)
    ...
    print_metrics(
        ...
        request_rate=request_rate,
        max_concurrency=max_concurrency,
        accept_length=acceptance_length,
