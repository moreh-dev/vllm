...
class OpenAIServingResponses(OpenAIServing):
    ...
    async def render_responses_request(
        ...
        Delegates preprocessing to OpenAIServingRender, adding the
        engine-aware checks (LoRA model validation, engine health) and the
        stateful store lookups that are specific to the Responses API. The
        renderer itself stays stateless: ``prev_response`` and
        ``prev_messages`` are looked up here and passed in as parameters.
        """
        ...
        # Handle the previous response ID. The store lookup stays here
        # (not in the renderer) to keep the renderer stateless.
